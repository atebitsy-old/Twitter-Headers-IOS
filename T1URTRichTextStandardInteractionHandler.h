//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <T1Twitter/T1URTRichTextInteractionHandler-Protocol.h>

@class NSString, TFSTwitterScribeContext;
@protocol TFSTwitterLegacyScribeLogMethods;

@interface T1URTRichTextStandardInteractionHandler : NSObject <T1URTRichTextInteractionHandler>
{
    // Error parsing type: , name: viewController
    // Error parsing type: , name: account
    // Error parsing type: , name: scribe
    // Error parsing type: , name: scribeContext
    // Error parsing type: , name: scribeAction
}

- (void).cxx_destruct;
- (id)init;
- (void)userDidLongPressIn:(id)arg1 range:(id)arg2 rect:(struct CGRect)arg3;
- (void)userDidTapWithRange:(id)arg1 rect:(struct CGRect)arg2;
- (id)initForViewController:(id)arg1 withAccount:(id)arg2;
@property(nonatomic, copy) NSString *scribeAction; // @synthesize scribeAction;
@property(nonatomic, retain) TFSTwitterScribeContext *scribeContext; // @synthesize scribeContext;
@property(nonatomic, retain) id <TFSTwitterLegacyScribeLogMethods> scribe; // @synthesize scribe;

@end

