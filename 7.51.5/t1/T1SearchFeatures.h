//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSString;
@protocol TFNTwitterFeatureSwitches;

@interface T1SearchFeatures : NSObject
{
    id <TFNTwitterFeatureSwitches> _featureSwitches;
}

@property(readonly, nonatomic) id <TFNTwitterFeatureSwitches> featureSwitches; // @synthesize featureSwitches=_featureSwitches;
- (void).cxx_destruct;
@property(readonly, nonatomic) unsigned long long includedSearchTabs;
@property(readonly, nonatomic) unsigned long long recentUserRecordingBucket;
@property(readonly, nonatomic, getter=isQualityFilterOptionAvailable) _Bool qualityFilterOptionAvailable;
@property(readonly, nonatomic) NSString *emptySearchResultsSearchSettingsLinkURLString;
- (id)init;
- (id)initWithFeatureSwitches:(id)arg1;

@end

